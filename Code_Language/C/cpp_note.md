...
### 0.4.6 priority_queue(优先序列,也称为堆,一个容器.本质是用一个数组去模拟的完全二叉树.可以拿出优先级最大的元素.)
...
在讨论static等概念前, 我们需要讨论一下变量/对象的一个属性: linkage链接性.

这个属性有三种情况:
| 链接类型                 | 英文名  | 定义                                       | 说明                        |
| -------------------- | ---- | ---------------------------------------- | ------------------------- |
| **external linkage** | 外部链接 | 该标识符可以在多个翻译单元（.c 文件）中共享，链接器会将同名对象视为同一个实体 | 例如非 static 的全局变量/函数   |
| **internal linkage** | 内部链接 | 该标识符仅在**当前翻译单元.c文件**内可见，不能被其他文件访问     | 例如 `static` 修饰的全局变量/函数    |
| **no linkage**       | 无链接  | 标识符只在其作用域内有效，不能与任何其他声明关联                 | 所有普通的函数内局部变量、函数内的**局部静态变量** |


## 0.9 static 关键字

它有3个作用. 这也意味着我们说`静态变量`的时候是多义的.
...
`static`使它拥有**全局变量的内存特征**.

* 内存区: 被分配到**静态存储区**而不是**栈**, 不随着一次函数调用返回而释放. **从而生命周期为整个程序.**
* 初始化后是唯一的. 下次调用foo的时候, 会记得该静态局部变量的值.
* **链接性无变化: 和普通的局部变量一样, 仍然不允许被其他翻译单元引用.**
...
******************
上述`static`两个作用的总结:
* **`static`作用到局部变量上, 会让它拥有全局变量的内存特征和生命周期, 其他诸如链接性不变.**
* **`static`作用到全局变量/函数上, 会收窄它的链接性为内部链接, 其他诸如内存特征生命周期不变.**
...
在函数签名的参数前面加上关键字`const`, 表达这个传进来的形式参数在该函数体内部不可被改变. 
```cpp
...
在传入某个真实数据时一般传入const 形式参数, 防止函数体内不小心把这个参数改变了.

对于指针的情况:

以函数foo的签名为例:

`const int *src`中, `const`修饰的是`src`. 编译器不允许你在函数体内通过指针`*src`修改该内存区域.

```C
void foo(const int *src, ...) {

	s[0] = 1;      // ❌ 编译错误：const 指针不能写入
    s[100] = 2;    // ❌ 同样编译错误. 编译器并不知道src指针对应的那块内存有多大, 所以只要通过src指针访问之后的内存都被禁止
    
    int x = s[0];  // ✅ 编译通过：读取是允许的
...
- C++中, class中提供成员/方法的`static`关键字: (属于类而不是具体的某个实例, 通过某个实例访问这个静态变量, 其实都是同一个变量)和静态方法(静态变量的函数). 
...
C语言中, **label直接对应汇编语言的符号symbol**.  它用于goto:

```C
foo:
  printf("hello"); //repeat hello
  goto foo;
```

我们知道, symbol符号在**链接阶段**会被记录在ELF文件的**符号表**里, 比如这样:

```
LabelName       Address
----------       --------
main             0x555555554620
loop_start       0x555555554660
exit_func        0x5555555546A0
...
```

C语言中, 一个label是一个单纯的标识符, 没有数据类型.

约定label的指针的数据类型为`void *`.


## 2. label指针: GCC扩展.


* GCC扩展支持**label指针**. 即, 你可以拿到一个label具体指向的地址.
```C
foo:
  printf("hello"); //repeat hello
  goto foo;
...
C语言中对label取指要用双取指符`&&`. 这是GCC专门扩展的. 为啥要区分?

* `&`取变量地址. 它们在内存的`栈区`(局部变量)或`数据段`(全局和静态变量)
* `&&`取指令地址. 它们在内存中的`代码段`.
...
| 名称                                   | 作用                       | 举例                                   |
| ------------------------------------ | ------------------------ | ------------------------------------ |
| **代码段（Text Segment / Code Segment）** | 存放可执行指令（CPU执行的机器码）       | 函数体、`main()`、`decode_exec()` 的指令都在这里 |
| **数据段（Data Segment）**                | 存放**已初始化的全局变量**与**静态变量** | 如 `int x = 3; static int a = 2;`     |
| **BSS 段（Block Started by Symbol）**   | 存放**未初始化的全局变量**与**静态变量** | 如 `int y; static int b;`             |
| **堆区（Heap）**                         | 动态分配的内存                  | 来自 `malloc()`、`new`                  |
| **栈区（Stack）**                        | 局部变量、函数调用栈帧              | 每次函数调用都会创建栈帧，保存局部变量与返回地址             |
| **内核空间（Kernel Space）**               | 操作系统内核代码与数据              | 用户态程序一般不可直接访问                        |
...
宏的内容是一个**标识符**. 不能直接当作字符串或者数字.

**标识符要么绑定在一个变量上，要么直接作为 label 来使用。**


```C
#define foo my_var
printf("%s\n", my_var);	//错误! myvar只是一个标识符. 还没有绑定到某种数据类型的data.

char* my_var = "helloworld";
printf("%s\n", my_var);	//现在就正确啦.
```
...
前者是简单的**标识符替换**. 后者会在编译时替换参数.
```C
#define fun 
```	
当定义内容出现`()`后, 会被自动为视为函数宏.
...
#define B _var
#define CAT(a, b) a##b
#define EXPAND(a, b) CAT(a, b) //这里其实等于给CAT一个别名叫做EXPAND

int main(){
	char* my_var = "helo world!";
	printf("%s\n", EXPAND(A,B));
	return 0;
}
...
**C23 新标准（草案阶段）引入“命名参数”语法**.

它借鉴了 Python、Rust、Kotlin 那类语言的写法，让函数调用时参数可以写成 带名字的形式，更直观可读。
...
可以看到下图中ai提示加一个`format:`, 但其实是不需要的. 这是因为你用了宏后, 编译器不知道后面的宏展开式什麽. 在他看来你写的是
```C
printf("%s\n", something)
```
