...
我已经厌倦了需要复杂的配置才能让插件`C/C++`正确解析和跳转我的C代码. 你必须详细配置including path; source path...才可以.

对于一个可以编译的项目, 这些明明可以被自动做好.

让我们下载并`clangd`插件. 同时在命令行安装`bear`. 它是clangd专门用来记录一个可编译项目的文件结构关系的.

`clangd`其实是一组包括**clangd编译器**在内的工具链的统称. `clangd`插件只是个壳子. 第一次使用, 会弹出提示:
* 请下载`clangd`编译器(同意即可)
* 如果你已经启用了`C/C++`, 会提示是否关闭它的功能. 关掉即可.

简单使用:

进入你的项目目录`nemu`.

进行一次完整项目构建, 同时用bear来记录得到文件依赖关系:

`bear -- [你的make构建命令, 比如make run -B]`

本次构建速度会很慢.

构建好后, 在`./nemu`下会生成一个`compile_commands.json`文件. 有了这个文件, clangd插件利用clangd编译器服务器, 可以正确地解析和跳转你的整个`nemu`项目里大大小小的各种变量和宏. enjoy!

只要没有大的修改和依赖关系变化和新增编译文件, 你就可以一劳永逸了. 否则再`bear -- [你的make构建命令, 比如make run -B]`一下就可以了.
