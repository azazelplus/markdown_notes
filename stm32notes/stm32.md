...
##### 2.GPIO的输出电路: 三种输出状态, 推挽输出和开漏输出模式
GPIO输出电路的内部示意图: (假设GPIO口接在一个简单的mos开关栅极上)
...
控制器通过控制`Q1`和`Q2`来改变GPIO的输出模式.

可以看到, 通过排列组合`Q1`和`Q2`(但是不能都打开会短路), 得到三种输出情况:
* Q1打开Q2关闭, GPIO的VCC给外部充电. 输出高电平(push)
* Q1关闭Q2打开, GPIO连接的外部电路的电源通过Q2放电. 输出低电平(pull)
...
只允许`push`和`pull`模式.
  ...
  * **开漏输出模式**: 
只允许`pull`和`浮空`模式. 该模式下`Q1`永远关闭, 所以电路图可以退化为:

![alt text](image-126.png)

此时`Q2`的漏极啥也没接所以称为开漏. 显然, 离开了VCC, 开漏输出本身不具有输出高电平的能力, 需要外接高电平.* 部分单片机已经配置好了电压源-上拉电阻.
*
**应用**:
