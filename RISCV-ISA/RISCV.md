...
它由多个PTE(page table entry)行组成. 每个PTE记录一个**虚拟页**映射到到一个**物理页**的信息, 占32bit.
...
MMU分解该虚拟地址为三部分:
* 页内偏移: **Offset** = 低12位`0x678`
* 虚拟页号: 高20位`0x12345` , 拆解为 
  * **VPN[1]**： 取 `0x12345` 的最高10位 (bits \[19:10\])。`0x12345 >> 10 = 0x48`   
  * **VPN[0]**： 取 `0x12345` 的最低10位 (bits \[9:0\])。`0x12345 & 0x3FF = 0x345`
...
-   **定位页表基址**：
    ...
    -   CPU中有一个名为 **`satp`** 的寄存器。操作系统在调度程序A运行时，会将程序A的**一级页表（页目录）的物理基地址**设置到 `satp` 寄存器中。
...
## 6.5 riscv32的可选扩展: **物理内存保护PMP**

理论上, riscv32的可选扩展: **物理内存保护PMP**支持34位物理地址.
