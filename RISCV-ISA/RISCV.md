...
#### 1.7.1.1  **结构冒险 (Structural Hazard)**
- 硬件资源不够导致冲突。 例如：取指和访存同时都要访问内存，但 CPU 只有一个端口，冲突了。  

#### 1.7.1.2  **数据冒险 (Data Hazard)**
...
#### 1.7.1.3 数据前递/旁路转发(Forwarding/Bypassing)
...
#### 1.7.1.4 气泡stall法: 流水线暂停几个周期，等数据准备好再继续。
...
#### 1.7.1.6 分支延迟槽 Branch Delay Slot
...
#### 1.7.1.7 **分支预测(Branch Prediction)**
...
#####  1.7.1.7.1 **静态预测 (Static Prediction)**
...
#####  1.7.1.7.2 **动态预测 (Dynamic Prediction)**
...
### 2.2.1 B-type指令 Branch-type 分支跳转
