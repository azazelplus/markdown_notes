...
组相联: 将cache行分成多个**组**, 每个组有多路cache行.
...
**对主存的写入，仅在缓存块被替换时发生。在此之前的任何CPU写操作，都只修改缓存副本.**

**这意味着，在很长一段时间内，缓存中的数据与主存中对应的数据是不一致的。**

CPU 要写入地址 `A`.

cpu先在缓存中寻找地址`A`的块, 如果**写命中**, 对缓存块写入, 并设置脏位为1. 不对主存操作. 主存此时存储旧数据.

如果写缺失: 去主存访问地址`A`. 然后将该内存块放入缓存. (放入方式根据映射规则: 直接映射/全相联/组相联, 和替换算法(LRU等)具体实现)
