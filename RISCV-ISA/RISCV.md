...
## 6.4 RISCV的地址转换: SV32/SV39 页地址转换
...
***   
-   **bits 31..20** : `PPN[1]`（12 bit）
-   **bits 19..10** : `PPN[0]`（10 bit）  其中低2位是保留位.  PTE的前22bit是
...
单级页表结构`一个页表由多个PTE组成`
还不能直接使用. 毕竟现代cpu同时有很多进程在运行. 

所以进一步引入二级页表的结构. 将4GB空间分为1024个块.
...
它是物理内存的一部分. 惯例是从 `0x10 0000` 到 `0x10 0FFF`, 一共4096字节.

每4字节是一个**PTE**

##### 6.4.2.2.1 satp寄存器(页表基寄存器)

首先, cpu中存在一个satp(supervisor address translaation & protection register)寄存器. 它的结构:
...
假设程序A要访问虚拟地址 `0x12345678`.

假设当前cpu的`satp.PPN` = `0x100`。这意味着程序A的一级页表存放在物理地址 `0x100000` (因为 0x100 << 12 = 0x100000)。





##### 6.4.2.2.3  二级页表结构的地址转换例子: riscv32 页地址转换
...
* **VPN**(虚拟页号): 高20位`0x12345` . 再截断为两部分:
  * **VPN[1]**： 取VPN的最高10位 `0x12345 >> 10 = 0x48`   为**一级索引**, 表示该虚拟地址所属的**页表的物理地址**距离**根页表物理基址**有多少个PTE(4字节)的偏移量. 也就是说, 该虚拟地址所属的**页表的物理地址**, 就是**根页表物理基址**加上(4*一级索引).
  * **VPN[0]**： 取VPN的最低10位 `0x12345 & 0x3FF = 0x345` 为**二级索引**, 表示
...
***
***
###### 2. 一级页表（root）查找: 读取并解析第一级页表项PTE1

我们需要通过**根页表物理基址** 和 **虚拟地址的一级索引** 来计算出PTE1的物理地址在哪里, 从而访问它.

* 根页表物理基址
  * 假设现在cpu的`satp.PPN`= `0x123`.
  * 那麽**根页表物理基址** = `satp.ppn << 12 = 0x123000`.
...
首先, **物理页基址**为PPN2 << 12 = `0x54321 << 12` = `0x54321000`, 偏移量为
...
-   **定位页表基址**：
    ...
    -   CPU中有一个名为 **`satp`** 的寄存器, 32bit宽. 操作系统在调度程序A运行时，会将程序A的**一级页表（页目录）的物理基地址**设置到 `satp` 寄存器中。
        
-   **第一级查找**：
    
    -   MMU根据 `satp` 中的基址，加上 `VPN[1] (0x48)` 作为索引（乘以4，因为每个页表项PTE是4字节），在物理内存中找到对应的**第一级页表项**。
        
    -   这个第一级PTE的内容**不是**最终的物理地址，而是**二级页表的物理基地址**。
        
-   **第二级查找**：
    
    -   MMU拿着从第一级找到的**二级页表基地址**，加上 `VPN[0] (0x345)` 作为索引（同样乘以4），在物理内存中找到对应的**第二级页表项**。
        
    -   这个**第二级PTE**中包含了我们最终想要的东西：**物理页框号**。
        
-   **获取物理页框号**：
    
    -   在SV32的PTE中，**PPN** 字段是 **22位**。它对应物理地址的 `[31:12]` 位。
        
    -   假设我们查到的 PPN 是 `0x00ABC`。
...
-   `0x00ABC << 12` 等于 `0x00ABC000`
    
-   `0x00ABC000 | 0x678` 等于 `0x00ABC678`
