...
* **2.隐式概率建模(更常见)**
  * 样本空间`X`存在一个能描述x的特征的隐变量空间`Z`. 比如, 看似多样的小动物图片集X, 每个图片都属于具体的小动物类别. 我们试图学习一个**带隐变量**的分布$p_φ(x|z)$. 例如**DDPM**.
...
### 概率probability和似然likelihood的区别:
概率是P(x|θ), 给定参数θ后样本x发生的概率
似然是L(θ|x), 给定样本x, 求`是参数θ让这个x出现`的概率.

### 对隐变量概率建模: 
描述整个样本空间的结构, 其实是x和z的联合分布p(x,z). 

# 5.变分推理

概率建模是大框架, 变分推理是概率建模的其中一个思想:
...
### 5.5. 变分下界 ELBO, 概率建模中重要的目标函数
我们看到上面的好多个模型, VAE,

在概率建模中, 如何选择**目标函数**呢? 我们通常想找到一个**最可能解释数据**的模型，也就是最大化**对数边缘似然 (log marginal likelihood)**：
$$
\log p(x)
$$
但这个公式通常**难以直接计算**，因为：
